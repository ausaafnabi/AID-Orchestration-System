class RoutersController < ApplicationController
  ...
  def load_routers
	  @routers_default = Gmaps4rails.build_markers(Router.all) do |plot,marker|
		  ...
		  @status = rand(1..4)
		  @ip = "192.168."+rand(0..255).to_s+"."+rand(15..220).to_s
		  ...
		  marker.infowindow renderer_to_string(:partial => "/routes/info",
						       :locals => {:name=>plot.name,:date => rand(6.months.ago..Time.now),:ip =>@ip,:connected=>@connected})
	  end
  end
